...
async function generateWithRetry(model, request, retries = 3) {
  let lastErr
  for (let i = 0; i < retries; i++) {
    try {
      ...
      const transient =
        s === 503 ||
        s === 429 ||
        /503|429|overloaded|high demand|UNAVAILABLE/i.test(msg)
...
export async function parseQuickInput(text, nowISO) {
  ...
  const result = await generateWithRetry(model, `現在日時: ${now}\n\n入力:\n${text}`)
